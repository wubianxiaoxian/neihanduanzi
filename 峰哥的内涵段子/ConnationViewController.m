...
-(void)createRefreshView{
    //下拉刷新 给scollView 增加的类别 增补的方法
    //arc防止两个强引用 导致内存泄露
    //写一个若饮用weakSelf 这样在block 中 就不会强引用
    ...
    [self.tableView addRefreshFooterViewWithAniViewClass:[JHRefreshCommonAniView class] beginRefresh:^{
        ...
        //上拉加载的时候max _timestamp 应该是最后一条数据的时间
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [cell showDataWithModel:model jumpBlock:^{
        CommontViewController *commont=[[CommontViewController alloc]init];
        [self.navigationController pushViewController:commont animated:YES];
    }];
    return cell;
}
//动态的计算 cell 的行高
